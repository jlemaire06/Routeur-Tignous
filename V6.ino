...
int digV, prev_digV, digC;          // Digitized voltage [previous] and current values
...
void loop() 
{
  ...
  prev_V = V;            
  prev_filtV = filtV;  
  digV = analogRead(VPin);
  digC = analogRead(CPin);
  V = (float)digV - off; 
  C = (float)digC - off;
  filtV = 0.996*(prev_filtV + digV - prev_digV); // High pass filter
